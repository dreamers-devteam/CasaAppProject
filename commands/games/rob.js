...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (success) {
      ...
      await interaction.reply(`💰 | You successfully robbed **${robAmount} Pcash** from **${targetUser.username}**!`);
    } else {
      // Failed rob, pay the target
      if (user.cash < robAmount) {
        return interaction.reply({ content: `❌ | You don't have enough Pcash to pay if the rob fails!`, ephemeral: true });
      }
      user.cash -= robAmount;
      target.cash += robAmount;
      user.lastRob = Date.now();
      ...
      await interaction.reply(`❌ | You failed to rob **${targetUser.username}** and paid them **${robAmount} Pcash** as a penalty.`);
    }
  }
};
